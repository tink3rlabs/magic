...
|------------------------|---------------------|---------------------------------------------------|-----------------------------------------------------------------------------|
| In-memory              | `storage.MEMORY`    | An in-process slice                               | Tests, demos, prototypes. **All data is lost on process exit.**             |
...
No config keys. Use only for tests and prototypes.
...
- `adapter.Query(dest, statement, limit, cursor, params...)` — paginated raw query.
...
- **Memory** — data lost on restart, single process only, no transactions.
...
- **DynamoDB** — the Lucene compiler targets PartiQL; fuzzy search and JSON path access are intentionally not implemented. Equality, range, wildcards (`LIKE`), and boolean composition work.
