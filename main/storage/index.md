...
No config keys. Use only for tests and prototypes. Internally it's an in-memory SQLite database — so SQLite quirks apply, e.g. boolean Lucene filters must be written `done:1`, not `done:true` (see [Search](https://tink3rlabs.github.io/magic/main/lucene/index.md)).
...
Values are escaped when the connection string is built, so passwords may contain spaces, quotes, `@`, `/` or `?`. For MySQL, driver options can follow the database name: `"dbname": "app?parseTime=true"`.

Quote secrets in YAML

If the config map comes from a YAML file, quote the values. Unquoted, `0123` is read as the number `83`, `~` as null, and `p@ss #x` as `p@ss`. Single quotes keep everything literal: `password: 'p@ss #x'`.
...
CosmosDB partition key is per-call, not global
...
- **`Search(dest, sortKey, query, limit, cursor, params...)`** — `query` is a Lucene query string. See [Search (Lucene)](https://tink3rlabs.github.io/magic/main/lucene/index.md).
